...
def get_plotly_fig(pandas_df, original_df):
    fig = px.scatter_3d(
        ...
        range_x=[math.floor(original_df["X"].min()), math.ceil(original_df["X"].max())],
        range_y=[math.floor(original_df["Y"].min()), math.ceil(original_df["Y"].max())],
        range_z=[math.floor(original_df["Z"].min()), math.ceil(original_df["Z"].max())],
    )
...
def main():
    ...
    options = [{"label": k, "value": k} for k in df["ORIGINAL_LABEL"].unique()]
    dropdown_menu_items = [{"label": k, "value": k} for k in sql_db.get_all_table_names()]
        ...
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.Div(
                                [
                                    html.H5(
                                        "Select the table you want to plot", style={"text-decoration": "underline"}
                                    ),
                                    dcc.Dropdown(
                                        id="select_table",
                                    ...
                                    html.H5(
                                        "Accuracy Under the current Settings", style={"text-decoration": "underline"}
                                    ),
                    ...
                    dbc.Col(
                        [
                            dcc.Graph(id="tsne_figure", figure=fig),
                            dcc.Slider(
                                id="epoch_slider",
                                min=df["epoch"].min(),
                                max=df["epoch"].max(),
    ...
    @app.callback(
        [
            ...
            Output("accuracy_info", "children"),
            Output("input_UID", "valid"),
            Output("input_UID", "invalid"),
        ],
        [
            Input("checklist", "value"),
            Input("tsne_figure", "relayoutData"),
